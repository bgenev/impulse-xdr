...
if [ "$PYTH_USE_SYST_VER" == "true" ]; then
    ...
    if [[ $OS_TYPE = "centos" || $OS_TYPE = "almalinux" ]]; then
    ...
    if [[ $OS_TYPE = "centos" || $OS_TYPE = "almalinux" || $OS_TYPE = "fedora"  ]]; then
...
if [ -x "$(command -v docker)" ]; then
	...
	if [[ $OS_TYPE = "linuxmint" ]]; then
		## Mint 
        DEBIAN_FRONTEND=noninteractive apt update -y
        DEBIAN_FRONTEND=noninteractive apt -y install apt-transport-https ca-certificates curl software-properties-common
        curl -fsSL https://download.docker.com/linux/ubuntu/gpg | sudo gpg --dearmor -o /usr/share/keyrings/docker-archive-keyring.gpg
