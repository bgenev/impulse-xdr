...
def stop_process(pid):
	try:
		...
		action_subprocess_success = True
		action_subprocess_msg = 'Process killed successfully.'		
	except:
		action_subprocess_success = False
...
def nft_block(ip_addr, state_action):
	...
	check_rule_in_table = 'nft list table inet impulse_table | grep "' + ip_addr + '"'
	rule_check = subprocess.run(check_rule_in_table, shell=True, capture_output=True, text=True).stdout.strip("\n")

	print(" *** check if rule exists in chain: ", rule_check)
